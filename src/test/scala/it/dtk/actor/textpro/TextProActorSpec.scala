...
class TextProActorSpec extends MySpec("TextProActorSpec") {
  ...
  "a Text Pro Actor" must {
    ...
    val textPro = system.actorOf(TextProActor.routerProps(1))

    "return a result when Parsing a news" in {
      textPro ! Parse(news)
      val n = expectMsgClass(classOf[News])
      n should have(
        'nlptitle ('defined),
        'nlpSummary ('defined),
        'nlpCorpus ('defined),
      ...
      n.nlpTags.value.size should be > 1
    }
